...
def execute_pam_call(context):
    context.grant_result = (
        context.peer.grant_token()
        .channels(list(context.resources_to_grant["CHANNEL"].values()))
        .groups(list(context.resources_to_grant["CHANNEL_GROUPS"].values()))
        .uuids(list(context.resources_to_grant["UUID"].values()))
        .ttl(context.TTL)
        .authorized_uuid(context.authorized_uuid)
        .sync()
    )
...
def step_impl(context, channel):
    ...
    context.publish_result = (
        context.peer_without_secret_key.publish()
