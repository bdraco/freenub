...
from pubnub.pubnub_asyncio import (
    EventEngineSubscriptionManager,
    PubNubAsyncio,
    SubscribeCallback,
)
...
async def test_subscribe():
    ...
    with patch.object(TestCallback, "status_callback") as status_callback, patch.object(
        TestCallback, "message_callback"
    ) as message_callback:
        ...
        await (
            ...
            .poll_delay(2)
            .poll_interval(1)
            .until_async(lambda: callback.message_called)
        )
        assert (
            pubnub._subscription_manager.event_engine.get_state_name()
            == states.ReceivingState.__name__
        )
...
async def test_handshaking():
    ...
    with patch.object(TestCallback, "status_callback") as status_callback:
        pubnub = PubNubAsyncio(
            config, subscription_manager=EventEngineSubscriptionManager
        )
        pubnub.add_listener(callback)
        pubnub.subscribe().channels("foo").execute()
        await (
            ...
            .poll_delay(2)
            .poll_interval(1)
            .until_async(lambda: callback.status_called)
        )
        assert (
            pubnub._subscription_manager.event_engine.get_state_name()
            == states.ReceivingState.__name__
...
async def test_handshake_failed_no_reconnect():
    ...
    assert (
        pubnub._subscription_manager.event_engine.get_state_name()
        == states.HandshakeFailedState.__name__
    )
    pubnub._subscription_manager.stop()
    await pubnub.close_session()
...
async def test_handshake_failed_reconnect():
    ...
    assert (
        pubnub._subscription_manager.event_engine.get_state_name()
        == states.HandshakeReconnectingState.__name__
    )
