...
class TestListPushProvisions(unittest.TestCase):
    ...
    def test_list_channel_group_apns(self):
        ...
        self.assertEqual(
            self.list_push.build_path(),
            ListPushProvisions.LIST_PATH % (pnconf.subscribe_key, "coolDevice"),
        )

        self.assertEqual(
            self.list_push.build_params_callback()({}),
            {"pnsdk": sdk_name, "uuid": self.pubnub.uuid, "type": "apns"},
        )
    ...
    def test_list_channel_group_gcm(self):
        ...
        self.assertEqual(
            self.list_push.build_params_callback()({}),
            {"pnsdk": sdk_name, "uuid": self.pubnub.uuid, "type": "gcm"},
        )
    ...
    def test_list_channel_group_mpns(self):
        ...
        self.assertEqual(
            self.list_push.build_params_callback()({}),
            {"pnsdk": sdk_name, "uuid": self.pubnub.uuid, "type": "mpns"},
        )

    def test_list_channel_group_apns2(self):
        self.list_push.push_type(PNPushType.APNS2).device_id("coolDevice").environment(
            pubnub.enums.PNPushEnvironment.PRODUCTION
        ).topic("testTopic")

        self.assertEqual(
            self.list_push.build_path(),
            ListPushProvisions.LIST_PATH_APNS2 % (pnconf.subscribe_key, "coolDevice"),
        )
    ...
    def test_apns2_default_environment(self):
        self.list_push.push_type(PNPushType.APNS2).device_id("coolDevice").topic(
            "testTopic"
        )
