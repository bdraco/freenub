...
class TestRemoveChannelsFromPush(unittest.TestCase):
    ...
    def test_push_remove_single_channel(self):
        self.remove_channels.channels(["ch"]).push_type(
            pubnub.enums.PNPushType.APNS
        ).device_id("coolDevice")

        params = (pnconf.subscribe_key, "coolDevice")
        self.assertEqual(
            self.remove_channels.build_path(),
            RemoveChannelsFromPush.REMOVE_PATH % params,
        )

        self.assertEqual(
            self.remove_channels.build_params_callback()({}),
            {
                "pnsdk": sdk_name,
    ...
    def test_push_remove_multiple_channels(self):
        self.remove_channels.channels(["ch1", "ch2"]).push_type(
    ...
    def test_push_remove_google(self):
        self.remove_channels.channels(["ch1", "ch2", "ch3"]).push_type(
    ...
    def test_push_remove_single_channel_apns2(self):
        ...
        self.assertEqual(
            self.remove_channels.build_path(),
            RemoveChannelsFromPush.REMOVE_PATH_APNS2 % params,
        )
