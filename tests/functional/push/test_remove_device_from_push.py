...
class TestRemoveDeviceFromPush(unittest.TestCase):
    ...
    def test_remove_push_apns(self):
        self.remove_device.push_type(pubnub.enums.PNPushType.APNS).device_id(
            "coolDevice"
        )

        params = (pnconf.subscribe_key, "coolDevice")
        self.assertEqual(
            self.remove_device.build_path(), RemoveDeviceFromPush.REMOVE_PATH % params
        )

        self.assertEqual(
            self.remove_device.build_params_callback()({}),
            {
                "pnsdk": sdk_name,
    ...
    def test_remove_push_gcm(self):
        self.remove_device.push_type(pubnub.enums.PNPushType.GCM).device_id(
            "coolDevice"
        )
    ...
    def test_remove_push_mpns(self):
        self.remove_device.push_type(pubnub.enums.PNPushType.MPNS).device_id(
            "coolDevice"
        )
    ...
    def test_remove_push_apns2(self):
        self.remove_device.push_type(pubnub.enums.PNPushType.APNS2).device_id(
            "coolDevice"
        ).environment(pubnub.enums.PNPushEnvironment.PRODUCTION).topic("testTopic")

        params = (pnconf.subscribe_key, "coolDevice")
        self.assertEqual(
            self.remove_device.build_path(),
            RemoveDeviceFromPush.REMOVE_PATH_APNS2 % params,
        )
