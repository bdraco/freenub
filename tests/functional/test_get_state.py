...
class TestGetState(unittest.TestCase):
    ...
    def test_get_state_single_channel(self):
        ...
        self.assertEqual(
            self.get_state.build_path(),
            GetState.GET_STATE_PATH % (pnconf.subscribe_key, "ch", self.pubnub.uuid),
        )
    ...
    def test_get_state_single_group(self):
        ...
        self.assertEqual(
            self.get_state.build_path(),
            GetState.GET_STATE_PATH % (pnconf.subscribe_key, ",", self.pubnub.uuid),
        )

        self.assertEqual(
            self.get_state.build_params_callback()({}),
            {"pnsdk": sdk_name, "uuid": self.pubnub.uuid, "channel-group": "gr"},
        )
