...
class TestGrant(unittest.TestCase):
    ...
    def test_grant_read_and_write_to_channel(self):
        ...
        self.assertEqual(
            self.grant.build_path(), Grant.GRANT_PATH % pnconf_pam.subscribe_key
        )
        ...
        sign_input = (
            HttpMethod.string(self.grant.http_method()).upper()
            + "\n"
        ...
        self.assertEqual(
            self.grant.build_params_callback()({}),
            {
                ...
                "channel": "ch",
                "signature": "v2."
                + utils.sign_sha256(pnconf_pam.secret_key, sign_input).rstrip("="),
            },
        )
