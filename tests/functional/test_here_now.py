...
class TestHereNow(unittest.TestCase):
    ...
    def test_here_now(self):
        ...
        self.assertEqual(
            self.here_now.build_path(),
            HereNow.HERE_NOW_PATH % (pnconf.subscribe_key, "ch1"),
        )
    ...
    def test_here_now_groups(self):
        ...
        self.assertEqual(
            self.here_now.build_path(),
            HereNow.HERE_NOW_PATH % (pnconf.subscribe_key, ","),
        )

        self.assertEqual(
            self.here_now.build_params_callback()({}),
            {"channel-group": "gr1", "pnsdk": sdk_name, "uuid": self.pubnub.uuid},
        )

    def test_here_now_with_options(self):
        self.here_now.channels(["ch1"]).channel_groups("gr1").include_state(
            True
        ).include_uuids(False)
