...
from pubnub.endpoints.channel_groups.remove_channel_from_channel_group import (
...
class TestRemoveChannelToChannelGroup(unittest.TestCase):
    ...
    def test_remove_single_channel(self):
        ...
        self.assertEqual(
            self.remove.build_path(),
            RemoveChannelFromChannelGroup.REMOVE_PATH % (pnconf.subscribe_key, "gr"),
        )

        self.assertEqual(
            self.remove.build_params_callback()({}),
            {"pnsdk": sdk_name, "uuid": self.pubnub.uuid, "remove": "ch"},
        )
    ...
    def test_remove_multiple_channels(self):
        ...
        self.assertEqual(
            self.remove.build_params_callback()({}),
            {"pnsdk": sdk_name, "uuid": self.pubnub.uuid, "remove": "ch1,ch2"},
        )
