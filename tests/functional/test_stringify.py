...
from pubnub.models.consumer.access_manager import (
...
from pubnub.models.consumer.history import PNHistoryItemResult, PNHistoryResult
from pubnub.models.consumer.presence import (
    PNGetStateResult,
...
from pubnub.models.consumer.push import (
...
class TestStringify(unittest.TestCase):
    def test_publish(self):
        assert (
            str(PNPublishResult(None, 123123))
            == "Publish success with timetoken 123123"
        )
    ...
    def test_remove_channel_from_group(self):
        assert (
            str(PNChannelGroupsRemoveChannelResult()) == "Channel successfully removed"
        )
    ...
    def test_list_channel_group(self):
        result = PNChannelGroupsListResult(["qwer", "asdf", "zxcv"])

        assert str(result) == "Group contains following channels: qwer, asdf, zxcv"

    def test_audit(self):
        result = PNAccessManagerAuditResult(
            None, None, None, None, None, 3600, True, False, True, False
        )

        assert str(result) == "Permissions are valid for 3600 minutes"

    def test_grant(self):
        result = PNAccessManagerGrantResult(
            None, None, None, None, None, 3600, True, False, True, False
        )
    ...
    def test_history(self):
        assert (
            str(PNHistoryResult(None, 123, 789)) == "History result for range 123..789"
        )
    ...
    def test_history_item(self):
        assert (
            str(
                PNHistoryItemResult(
                    {"blah": 2}, PubNubCryptodome(PNConfiguration()), 123
                )
            )
    ...
    def test_push_list(self):
        assert (
            str(PNPushListProvisionsResult(["qwer", "asdf"]))
            == "Push notification enabled on following channels: qwer, asdf"
        )

    def test_push_add(self):
        assert str(PNPushAddChannelResult()) == "Channel successfully added"

    def test_push_remove(self):
        assert str(PNPushRemoveChannelResult()) == "Channel successfully removed"

    def test_push_remove_all(self):
        assert (
            str(PNPushRemoveAllChannelsResult()) == "All channels successfully removed"
        )
