...
class TestSubscribe(unittest.TestCase):
    def setUp(self):
        self.pubnub = MagicMock(
            spec=PubNub, config=pnconf, sdk_name=sdk_name, _get_token=lambda: None
        )
    ...
    def test_pub_single_channel(self):
        ...
        self.assertEqual(
            self.sub.build_path(),
            Subscribe.SUBSCRIBE_PATH % (pnconf.subscribe_key, "ch"),
        )

        self.assertEqual(
            self.sub.build_params_callback()({}),
    ...
    def test_sub_multiple_channels_using_string(self):
        ...
        self.assertEqual(
            self.sub.build_path(),
            Subscribe.SUBSCRIBE_PATH % (pnconf.subscribe_key, "ch1,ch2,ch3"),
        )
    ...
    def test_sub_single_group(self):
        ...
        self.assertEqual(
            self.sub.build_path(),
            Subscribe.SUBSCRIBE_PATH % (pnconf.subscribe_key, ","),
        )

        self.assertEqual(
            self.sub.build_params_callback()({}),
            {"channel-group": "gr", "pnsdk": sdk_name, "uuid": self.pubnub.uuid},
        )
    ...
    def test_sub_multiple(self):
        self.sub.channels("ch1,ch2").filter_expression("blah").region(
            "us-east-1"
        ).timetoken("123")

        self.assertEqual(
            self.sub.build_path(),
            Subscribe.SUBSCRIBE_PATH % (pnconf.subscribe_key, "ch1,ch2"),
        )
