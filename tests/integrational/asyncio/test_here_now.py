...
async def test_single_channel(event_loop, sleeper=asyncio.sleep):
    ...
    env = await pubnub.here_now().channels(ch).include_uuids(True).future()
    ...
    result = await pubnub.here_now().channels(ch).include_state(True).result()
...
async def test_multiple_channels(event_loop, sleeper=asyncio.sleep):
    ...
    env = await pubnub.here_now().channels([ch1, ch2]).future()
    ...
    result = await pubnub.here_now().channels([ch1, ch2]).include_state(True).result()
...
@pn_vcr.use_cassette(
    "tests/integrational/fixtures/asyncio/here_now/global.yaml",
    filter_query_parameters=["pnsdk", "l_pres"],
    match_on=["method", "scheme", "host", "port", "string_list_in_path", "query"],
    match_on_kwargs={"string_list_in_path": {"positions": [4]}},
)
@pytest.mark.asyncio
