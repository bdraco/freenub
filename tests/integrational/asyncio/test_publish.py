...
from pubnub.pubnub_asyncio import AsyncioEnvelope, PubNubAsyncio, PubNubAsyncioException
...
async def test_publish_object_via_get(event_loop):
    ...
    await asyncio.ensure_future(
        assert_success_publish_get(pubnub, {"name": "Alex", "online": True})
    )
...
async def test_publish_mixed_via_post(event_loop):
    ...
    await asyncio.gather(
        ...
        asyncio.ensure_future(
            assert_success_publish_post(pubnub, ["hi", "hi2", "hi3"])
        ),
    )
...
async def test_publish_object_via_post(event_loop):
    pubnub = PubNubAsyncio(pnconf_copy(), custom_event_loop=event_loop)
    await asyncio.ensure_future(
        assert_success_publish_post(pubnub, {"name": "Alex", "online": True})
    )
...
async def test_publish_mixed_via_get_encrypted(event_loop):
    with patch(
        ...
        await asyncio.gather(
            ...
            asyncio.ensure_future(assert_success_publish_get(pubnub, True)),
            asyncio.ensure_future(
                assert_success_publish_get(pubnub, ["hi", "hi2", "hi3"])
            ),
        )
...
async def test_error_missing_message(event_loop):
    pubnub = PubNubAsyncio(pnconf_copy(), custom_event_loop=event_loop)
    await assert_client_side_error(
        pubnub.publish().channel(ch).message(None), "Message missing"
    )
...
async def test_error_missing_channel(event_loop):
    pubnub = PubNubAsyncio(pnconf_copy(), custom_event_loop=event_loop)
    await assert_client_side_error(
        pubnub.publish().channel("").message("hey"), "Channel missing"
    )
...
async def test_error_non_serializable(event_loop):
    ...
    await assert_client_side_error(
        pubnub.publish().channel(ch).message(method), "not JSON serializable"
    )
    await pubnub.stop()
...
async def test_publish_with_meta(event_loop):
    ...
    await assert_success_await(
        pubnub.publish().channel(ch).message("hey").meta({"a": 2, "b": "qwer"})
    )
    await pubnub.stop()
...
async def test_publish_do_not_store(event_loop):
    ...
    await assert_success_await(
        pubnub.publish().channel(ch).message("hey").should_store(False)
    )
    await pubnub.stop()
...
async def test_error_invalid_key(event_loop):
    ...
    await assert_server_side_error_yield(
...
async def test_not_permitted(event_loop):
    ...
    await assert_server_side_error_yield(
        pubnub.publish().channel(ch).message("hey"), "HTTP Client Error (403"
    )
    await pubnub.stop()
