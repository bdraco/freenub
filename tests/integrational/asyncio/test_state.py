...
from tests.helper import pnconf, pnconf_copy, pnconf_pam_copy, pnconf_sub_copy
...
async def test_single_channelx(event_loop):
    ...
    env = await pubnub.set_state().channels(ch).state(state).future()

    assert env.result.state["name"] == "Alex"
    assert env.result.state["count"] == 5

    env = await pubnub.get_state().channels(ch).future()
...
async def test_multiple_channels(event_loop):
    ...
    env = await pubnub.set_state().channels([ch1, ch2]).state(state).future()

    assert env.result.state["name"] == "Alex"
    assert env.result.state["count"] == 5

    env = await pubnub.get_state().channels([ch1, ch2]).future()
