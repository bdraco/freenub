...
from pubnub.models.consumer.objects_v2.channel import (
...
class TestObjectsV2Channel:
    ...
    _some_custom = {"key1": "val1", "key2": "val2"}
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/objects_v2/channel/set_channel.yaml",
    ...
    def test_get_all_channel_happy_path(self):
        ...
        pn.set_channel_metadata().include_custom(True).channel(
            TestObjectsV2Channel._some_channel_id
        ).set_name(TestObjectsV2Channel._some_name).description(
            TestObjectsV2Channel._some_description
        ).custom(TestObjectsV2Channel._some_custom).sync()

        get_all_channel_result = (
            pn.get_all_channel_metadata()
            .include_custom(True)
            .limit(10)
            .include_total_count(True)
            .sort(
                PNSortKey.asc(PNSortKeyValue.ID), PNSortKey.desc(PNSortKeyValue.UPDATED)
            )
            .page(None)
            .sync()
        )
