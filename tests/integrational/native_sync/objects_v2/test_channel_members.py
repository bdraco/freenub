...
from pubnub.endpoints.objects_v2.members.manage_channel_members import (
    ManageChannelMembers,
)
from pubnub.endpoints.objects_v2.members.remove_channel_members import (
...
class TestObjectsV2ChannelMembers:
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/objects_v2/channel_members/set_channel_members.yaml",
    ...
    def test_set_channel_members_happy_path(self):
        ...
        pn.set_uuid_metadata().uuid(some_uuid).set_name("some name").sync()

        custom_1 = {"key3": "val1", "key4": "val2"}
        pn.set_uuid_metadata().uuid(some_uuid_with_custom).set_name(
        ...
        assert (
            len(
                [
                    e
                    for e in data
                    if e["uuid"]["id"] == some_uuid
                    or e["uuid"]["id"] == some_uuid_with_custom
                ]
            )
            == 2
        )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/objects_v2/channel_members/get_channel_members.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_get_channel_members_happy_path(self):
        ...
        get_channel_members_result = (
            pn.get_channel_members()
            .channel(TestObjectsV2ChannelMembers._some_channel_id)
            .include_custom(True)
            .include_uuid(UUIDIncludeEndpoint.UUID_WITH_CUSTOM)
            .sync()
    ...
    def test_get_channel_members_with_pagination(self):
        ...
        pn.set_channel_members().channel(
            TestObjectsV2ChannelMembers._some_channel_id
        ).uuids([JustUUID(f"test-fix-118-{x}") for x in range(15)]).sync()
        ...
        assert isinstance(
            get_channel_members_result_page_1.result, PNGetChannelMembersResult
        )
        ...
        get_channel_members_result_page_2 = (
            pn.get_channel_members()
            .channel(TestObjectsV2ChannelMembers._some_channel_id)
            .limit(10)
        ...
        assert isinstance(
            get_channel_members_result_page_2.result, PNGetChannelMembersResult
        )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/objects_v2/channel_members/"
    ...
    def test_remove_channel_members_happy_path(self):
        ...
        remove_channel_members_result = (
            pn.remove_channel_members()
            .channel(TestObjectsV2ChannelMembers._some_channel_id)
        ...
        assert isinstance(
            remove_channel_members_result.result, PNRemoveChannelMembersResult
        )
    ...
    def test_manage_channel_members_happy_path(self):
        ...
        assert isinstance(
            manage_channel_members_result.result, PNManageChannelMembersResult
        )
