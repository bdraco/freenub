...
class TestPubNubChannelGroups(unittest.TestCase):
    ...
    def test_single_channel(self):
        ...
        # cleanup
        envelope = pubnub.remove_channel_group().channel_group(gr).sync()

        assert isinstance(envelope.result, PNChannelGroupsRemoveGroupResult)

        # add
        envelope = (
            pubnub.add_channel_to_channel_group().channels(ch).channel_group(gr).sync()
        )
        ...
        # list
        envelope = pubnub.list_channels_in_channel_group().channel_group(gr).sync()
