...
class TestFetchMessages:
    ...
    def test_fetch_messages_return_max_100_for_single_channel(self):
        ...
        while True:
            time.sleep(1)
            if (
                len(pubnub.history().channel(ch).count(COUNT).sync().result.messages)
                >= 100
            ):
                break
    ...
    def test_fetch_messages_return_max_25_for_multiple_channels(self):
        ...
        while True:
            time.sleep(1)
            if (
                len(pubnub.history().channel(ch1).count(COUNT).sync().result.messages)
                >= 100
                and len(
                    pubnub.history().channel(ch2).count(COUNT).sync().result.messages
                )
                >= 100
            ):
                break
    ...
    def test_fetch_messages_actions_return_max_25(self):
        ...
        envelope = (
            pubnub.fetch_messages().channels(ch).include_message_actions(True).sync()
        )
