...
from tests.helper import pnconf_enc_env_copy, pnconf_file_copy
...
def test_send_and_download_file_using_bytes_object(
    file_for_upload, file_upload_test_data
):
...
def test_file_exceeded_maximum_size(file_for_upload_10mb_size):
    ...
    assert "Your proposed upload exceeds the maximum allowed size" in str(
        exception.value
    )
...
def test_fetch_file_upload_s3_data(file_upload_test_data):
    envelope = (
        ...
        .channel(CHANNEL)
        .file_name(file_upload_test_data["UPLOADED_FILENAME"])
        .sync()
    )
...
def test_send_and_download_gcm_encrypted_file(file_for_upload, file_upload_test_data):
    ...
    with patch(
        ...
        envelope = send_file(
            file_for_upload, cipher_key=cipher_key, pubnub_instance=pubnub
        )
...
def test_send_and_download_encrypted_file_fallback_decode(
    file_for_upload, file_upload_test_data
):
    ...
    with patch(
        ...
        envelope = send_file(
            file_for_upload, cipher_key=cipher_key, pubnub_instance=pn_cbc
        )

        download_envelope = (
            pn_gcm.download_file()
            .channel(CHANNEL)
            .file_id(envelope.result.file_id)
            .file_name(envelope.result.name)
            .cipher_key(cipher_key)
