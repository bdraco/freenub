...
@pn_vcr.use_cassette(
    "tests/integrational/fixtures/native_sync/message_count/single.yaml",
...
@pn_vcr.use_cassette(
    "tests/integrational/fixtures/native_sync/message_count/multi.yaml",
    filter_query_parameters=["uuid", "seqn", "pnsdk"],
)
def test_multiple_channels(pn):
    ...
    time = envelope.result.timetoken - 10
    envelope = (
        pn.message_counts().channel(chans).channel_timetokens([time, time]).sync()
    )
