...
class TestPubNubPublish(unittest.TestCase):
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_string_get.yaml",
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_list_get.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_bool_get.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_publish_bool_get(self):
        try:
            env = PubNub(pnconf).publish().channel("ch1").message(True).sync()
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_int_get.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_publish_int_get(self):
        try:
            env = PubNub(pnconf).publish().channel("ch1").message(5).sync()
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_string_post.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_list_post.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_object_post.yaml",
        filter_query_parameters=["uuid", "seqn", "pnsdk"],
        match_on=[
            "method",
            "scheme",
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_bool_post.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_int_post.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_publish_int_post(self):
        try:
            env = (
                PubNub(pnconf).publish().channel("ch1").message(5).use_post(True).sync()
            )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_encrypted_string_post.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_encrypted_list_post.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/invalid_key.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_invalid_key(self):
        ...
        try:
            PubNub(config).publish().channel("ch1").message("hey").sync()
    ...
    def test_missing_message_error(self):
        try:
            PubNub(pnconf).publish().channel("ch1").message(None).sync()
    ...
    def test_missing_channel_error(self):
        try:
            PubNub(pnconf).publish().channel("").message("hey").sync()
    ...
    def test_non_serializable_error(self):
        ...
        try:
            PubNub(pnconf).publish().channel("ch1").message(func).sync()
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_with_meta.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    ...
    def test_single_quote_character_message_encoded_ok(self):
        envelope = PubNub(pnconf).publish().channel("ch1").message('"').sync()

        assert envelope

    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_ttl_0.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_publish_ttl_0(self):
        try:
            env = PubNub(pnconf).publish().channel("ch1").message("hi").ttl(0).sync()
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/publish/publish_ttl_100.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
    )
    def test_publish_ttl_100(self):
        try:
            env = PubNub(pnconf).publish().channel("ch1").message("hi").ttl(100).sync()
