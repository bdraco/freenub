...
class TestPubNubState(unittest.TestCase):
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/state/state_of_single_channel.yaml",
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/state/state_of_multiple_channels.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
        match_on=["state_object_in_query"],
    )
    ...
    def test_super_call(self):
        ...
        env = pubnub.set_state().channels([ch1, ch2]).state(state).sync()

        assert env.result.state["name"] == "Alex"
        assert env.result.state["count"] == 5

        env = pubnub.get_state().channels([ch1, ch2]).sync()
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_sync/state/state_with_user_defined_uuid.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
        match_on=["state_object_in_query"],
    )
