...
class TestFileUploadThreads(unittest.TestCase):
    ...
    def test_send_file(self):
        ...
        pubnub.send_file().channel(CHANNEL).file_name(
            self.file_for_upload.basename
        ).message({"test_message": "test"}).should_store(True).ttl(222).file_object(
            fd
    ...
    def test_list_files(self):
        ...
        assert (
            self.file_upload_test_data["UPLOADED_FILENAME"]
            == self.response.data[0]["name"]
        )
    ...
    def test_send_and_download_file(self):
        ...
        pubnub.download_file().channel(CHANNEL).file_id(result.file_id).file_name(
            result.name
        ).pn_async(self.callback)
        ...
        assert (
            ...
            == self.file_upload_test_data["FILE_CONTENT"]
        )
    ...
    def test_delete_file(self):
        ...
        pubnub.delete_file().channel(CHANNEL).file_id(result.file_id).file_name(
            result.name
        ).pn_async(self.callback)
    ...
    def test_get_file_url(self):
        ...
        pubnub.get_file_url().channel(CHANNEL).file_id(result.file_id).file_name(
            result.name
        ).pn_async(self.callback)
    ...
    def test_fetch_file_upload_s3_data(self):
        pubnub._fetch_file_upload_s3_data().channel(CHANNEL).file_name(
            self.file_upload_test_data["UPLOADED_FILENAME"]
        ).pn_async(self.callback)
    ...
    def test_publish_file_message(self):
        PublishFileMessage(pubnub).channel(CHANNEL).meta({}).message(
            {"test": "test"}
        ).file_id("2222").file_name("test").should_store(True).ttl(222).pn_async(
            self.callback
        )
