...
import pubnub
from pubnub.models.consumer.presence import PNGetStateResult, PNSetStateResult
...
class TestPubNubState(unittest.TestCase):
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_threads/state/state_of_single_channel.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
        match_on=["state_object_in_query"],
    )
    def test_single_channel(self):
        ...
        pubnub.set_state().channels(ch).state(state).pn_async(self.callback)
        ...
        self.event.clear()
        pubnub.get_state().channels(ch).pn_async(self.callback)
    ...
    @pn_vcr.use_cassette(
        "tests/integrational/fixtures/native_threads/state/state_of_multiple_channels.yaml",
        filter_query_parameters=["uuid", "pnsdk"],
        match_on=["state_object_in_query"],
    )
    def test_multiple_channels(self):
        ...
        pubnub.set_state().channels([ch1, ch2]).state(state).pn_async(self.callback)
        ...
        self.event.clear()
        pubnub.get_state().channels([ch1, ch2]).pn_async(self.callback)
