...
pn_vcr = vcr.VCR(cassette_library_dir=vcr_dir)

pn_vcr_with_empty_body_request = vcr.VCR(
    cassette_library_dir=vcr_dir, before_record_request=remove_request_body
)
...
def object_in_body_matcher(r1, r2, decrypter=None):
    try:
        if decrypter is not None:
            ...
            assert json.loads(url_decode(r1.body.decode("utf-8"))) == json.loads(
...
def use_cassette_and_stub_time_sleep_native(cassette_name, **kwargs):
    ...
    full_path = f"{pn_vcr.cassette_library_dir}/{cassette_name}"
