...
from vcr.serializers.jsonserializer import deserialize, serialize
...
class PNSerializer:
    ...
    def __init__(self) -> None:
        self.envs = {
            key: value for key, value in os.environ.items() if key.startswith("PN_KEY_")
        }
    ...
    def replace_keys(self, uri_string):
        for pattern in self.patterns:
            ...
            if found and found.group(0) in list(self.envs.values()):
                key = list(self.envs.keys())[
                    list(self.envs.values()).index(found.group(0))
                ]
                if key:
    ...
    def deserialize(self, cassette_string):
        ...
        for index, interaction in enumerate(cassette_dict["interactions"]):
            if "binary" in interaction["response"]["body"].keys():
                interaction["response"]["body"]["string"] = b64decode(
                    interaction["response"]["body"]["binary"]
                )
