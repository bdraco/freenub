...
from tests.helper import hardcoded_iv_config_copy, pnconf_file_copy
...
class TestPubNubCryptodome:
    def test_decode_aes(self):
        ...
        assert (
            crypto.decrypt(KEY, crypto.encrypt(KEY, multiline_test_message))
            == multiline_test_message
        )
