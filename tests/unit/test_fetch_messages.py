...
class TestFetchMessages:
    def test_single_channel_always_pass_max_when_in_bounds(self):
        ...
        fetch_messages_endpoint_under_test.channels("channel1").count(
            expected_max_value
        )
    ...
    def test_single_channel_always_pass_default_when_non_positive(self):
        ...
        # then
        assert (
            fetch_messages_endpoint_under_test._count
            == EXPECTED_SINGLE_CHANNEL_DEFAULT_MESSAGES
        )
    ...
    def test_multiple_channels_always_pass_max_when_in_bounds(self):
        ...
        fetch_messages_endpoint_under_test = pubnub.fetch_messages()
        fetch_messages_endpoint_under_test.channels(["channel1", "channel2"]).count(
            expected_max_value
        )
    ...
    def test_multiple_channels_always_pass_default_when_non_positive(self):
        ...
        assert (
            fetch_messages_endpoint_under_test._count
            == EXPECTED_MULTIPLE_CHANNEL_DEFAULT_MESSAGES
        )
    ...
    def test_multiple_channels_pass_default_when_max_exceeds(self):
        # given
        expected_max_value = randrange(
            MULTIPLE_CHANNELS_MAX_FOR_FETCH_MESSAGES + 1, 1000
        )
    ...
    def test_single_channel_with_actions_pass_default_when_not_specified(self):
        # given
        fetch_messages_endpoint_under_test = pubnub.fetch_messages()
        fetch_messages_endpoint_under_test.channels("channel1").include_message_actions(
            True
        )
        ...
        assert (
            fetch_messages_endpoint_under_test._count
            == EXPECTED_DEFAULT_MESSAGES_WITH_ACTIONS
        )
